...
def training(shape):
  ...
  for cur_image_index in range(1,imageQuantity+1):
    cur_image_path = f"./data/svd_example{cur_image_index}.jpg"
...
def generate_image(path, sigma):
  cur_image = cv.imread(path)
  cur_image = grayscale(cur_image)
  return noise_generation(cur_image, sigma)
...
noised_image = generate_image(sys.argv[1], sigma)
cv.imwrite("noised.jpg", noised_image)
